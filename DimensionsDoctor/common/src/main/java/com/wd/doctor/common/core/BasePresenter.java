...
import com.wd.doctor.common.bean.BDResult;
import com.wd.doctor.common.bean.DataBean;
import com.wd.doctor.common.core.exception.ApiException;
import com.wd.doctor.common.core.exception.CustomException;
...
public abstract class BasePresenter<T> {
    ...
    public BasePresenter(Cinterface dataCall) {
        this.dataCall = dataCall;
        Class<T> tClass = getTClass();
        iRequest = NetWorkManager.getInstener().create(getRequestType(),tClass);
    }
    ...
    protected Consumer getConsumer(final Object...args){
        if (getResponseType()==RESPONSE_TYPE_SDK_BD) {//如果整个项目中只有一个百度的接口，那么不建议修改基类Presenter，请重写getConsumer方法就可以。
            ...
            return new Consumer<DataBean>() {
                @Override
                public void accept(DataBean result) throws Exception {
                    running = false;
